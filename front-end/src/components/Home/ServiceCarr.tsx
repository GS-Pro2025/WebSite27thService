...
const ServicesWheel: React.FC = () => {
  ...
  const centerCircleSize = wheelSize * 0.5;  // 250px / 500px
  const iconRingRadius = wheelSize * 0.44;   // 220px / 500px
  const iconSize = wheelSize * 0.28;         // 140px / 500px
  const iconImageSize = iconSize * 0.7;      // ~98px / 140px
  ...
  const handleCenterClick = () => {
    navigate("/services", { state: { scrollTo: "moving" } });
  };
  ...
  return (
    <>
      ...
      <div className="w-full flex items-center justify-center p-4">
        ...
        <div 
          ref={wheelRef}
          className="relative w-full aspect-square max-w-[300px] sm:max-w-[400px] lg:max-w-[500px]"
        >
          {/* Anillo exterior*/}
          ...
          >
            {services.map((service, index) => {
              ...
              return (
                <div
                  key={service.id}
                  className="absolute bg-[#0E6F7E] rounded-full flex items-center justify-center shadow-lg"
                  style={{
                    width: `${iconSize}px`,
                    height: `${iconSize}px`,
          ...
          <div 
            ...
            className="absolute transform -translate-x-1/2 -translate-y-1/2 bg-[#BAD2DD] rounded-full z-40 flex items-center justify-center text-[#848484] font-bold text-center shadow-2xl border-4 border-slate-300 cursor-pointer hover:bg-[#A5C4D0] transition-colors duration-300"
            style={{
              width: `${centerCircleSize}px`,
              height: `${centerCircleSize}px`,
              top: '50%',
              left: '50%',
              fontSize: `${centerCircleSize * 0.12}px`,
            }}
          >
            SERVICES
          </div>
