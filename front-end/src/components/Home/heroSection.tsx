  ...
  return (
    <section className="w-full bg-[#68A2A6] overflow-hidden relative">
      ...
      >
        ...
        {/* Título principal */}
        <h1 
          className={`font-['Montserrat'] text-white text-[16px] sm:text-xl md:text-2xl lg:text-3xl xl:text-[40px] leading-tight text-center -ml-28 sm:-ml-6 md:-ml-8 lg:-ml-50 transition-all duration-1000 delay-300 ${
            isLoaded ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-8'
          }`}
          style={{
            transform: `scale(${titleScale}) translateY(${isLoaded ? 0 : 32}px)`,
        ...
        >
          ...
          <br />
          <span className={`font-extrabold inline-block transition-all duration-700 delay-600 hover:text-[#FFE67B] ${
        ...
        <div 
          ...
          style={{
            transform: `translateY(${isLoaded ? 0 : 24}px) scale(${Math.max(0.9, 1 - scrollY * 0.0005)})`
          }}
        >
          <button 
            ...
            style={{
              boxShadow: `0 ${Math.max(4, 8 - scrollY * 0.01)}px ${Math.max(8, 16 - scrollY * 0.02)}px rgba(0,0,0,0.2)`
            }}
          >
        ...
        <p 
          ...
          style={{
            transform: `translateY(${isLoaded ? 0 : 16}px) scale(${Math.max(0.95, 1 - scrollY * 0.0003)})`,
        ...
        >
          ...
          <span className={`font-extrabold text-[#FFE67B] transition-all duration-500 hover:animate-pulse ${
            isLoaded ? 'opacity-100' : 'opacity-0'
